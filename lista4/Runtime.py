...
def duck_typing_test():
    ...
    data_list = [3, 2, 5, 1, 4, 6, 7, 9, 10, 8]

    accumualtor = MultipleAccumulate(data_list, sum, mul, min, max)

    viewer_creator = ViewerCreator()
    text_viewer1 = viewer_creator.create_viewer('resources/text_file1.txt')
    text_viewer2 = viewer_creator.create_viewer('resources/text_file2.txt')

    duck_typing_test_list = [accumualtor, text_viewer1, text_viewer2]
    for obj in duck_typing_test_list:
        print(obj.get_data())
